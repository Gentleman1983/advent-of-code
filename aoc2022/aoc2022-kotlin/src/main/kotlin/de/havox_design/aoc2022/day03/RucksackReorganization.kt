...
class RucksackReorganization(private val filename: String) {
    fun getDuplicatesFromList(): List<Item> {
        ...
        if (!input.isNullOrEmpty()) {
            for (knapsackContent: String in input) {
                duplicates += Knapsack.getKnapsackForString(knapsackContent)
                    .findDuplicateItems()
            }
        }
    ...
    fun getDuplicatesScoreFromList(): Int {
        ...
        if (!input.isNullOrEmpty()) {
            for (knapsackContent: String in input) {
                score += Knapsack.getKnapsackForString(knapsackContent)
    ...
    fun detectBadgesFromList(): List<Item> {
        ...
        if (!input.isNullOrEmpty()) {
            for (index in 0..(input.size - 1) / 3) {
                ...
                val itemsTeam1: Set<Item> = Knapsack.getKnapsackForString(input[idxTeam1])
                    .listDistinctItems()
                val idxTeam2: Int = index * 3 + 1
                val itemsTeam2: Set<Item> = Knapsack.getKnapsackForString(input[idxTeam2])
                    .listDistinctItems()
                val idxTeam3: Int = index * 3 + 2
                val itemsTeam3: Set<Item> = Knapsack.getKnapsackForString(input[idxTeam3])
                    .listDistinctItems()
