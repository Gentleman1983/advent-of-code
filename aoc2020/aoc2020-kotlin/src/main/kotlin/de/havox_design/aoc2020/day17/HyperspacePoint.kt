...
class HyperspacePoint constructor(val parts: IntArray) {
    ...
    operator fun plus(other: IntArray) =
        HyperspacePoint(IntArray(minOf(size, other.size)) { parts[it] + other[it] })
    ...
    private fun neighbourOffsets(dimension: Int): List<IntArray> {
        return if (dimension == 0) {
            ...
            neighbourOffsets(dimension - 1)
                .flatMap { sequenceOf(it + -1, it + 0, it + 1) }
        }
    }

    override fun equals(other: Any?) =
        other is HyperspacePoint && parts.contentEquals(other.parts)
    ...
    companion object {
        fun of(vararg parts: Int) =
            HyperspacePoint(parts)

        fun zero(dimensions: Int) =
            HyperspacePoint(IntArray(dimensions) { 0 })
