...
public class TheHaltingProblem implements AoCFunctionality {
    ...
    private long getDiagnosticChecksum() {
        ...
        char nextStateName = Helper.getCharacter(iterator.next());
        int iterations = Helper.getNumber(iterator.next());
        Map<Character, State> states = Helper.getStateMap(iterator);
